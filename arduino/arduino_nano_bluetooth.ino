/* 
 *  HomeCentral_Circuit3_003_HC05.ino
 ...
 *  - Reads data from the HomeCentral smartphone app over a Bluetooth connection.
 *  - Receives control signals from Circuit Two over an I2C connection
 *    This is the slave in the I2C connection.
 *  
...
/* 
 *  For HC-05 use software serial with GPIO pins
 *  RX and TX on the board connect to TX and RX respectively on the HC05
 */
#define RX_pin 7
#define TX_pin 8

/* 
 * I2C address for Circuit Three 
 * Some addresses are reserved for system use and some addresses
 * are used for components as a convention. Ensure there is no conflict
 * with any of the components in your circuit.
 */
...
char control_signal_bt[8]; //control signal received via Bluetooth from app
char control_signal_i2c[8]; //control signal received via I2C from Circuit Two
 ...
 * Control signals should be received as "device_id | on" or "device_id | off"
 * Could be over Bluetooth or from Circuit Two over the I2C connection
 */
void processControlSignal(char control_signal[]){
  Serial.println(control_signal);

  //Get the device_id and the state from the pipe-separated string
...
void onReceiveI2CData() {
  int i = 0; //counter to increment char array
  ...
  //Call the function to process the control signal
...
void setup() {
  ...
  //sets baud rate (required), 
  //and data, parity, and stop bits (optional)
  BTSerial.begin(9600);
  //INFO: on the serial monitor
  Serial.println("HC-05 Bluetooth initialized!");

  //Start the I2C communication as a slave
  //Takes this device I2C address as a parameter
...
void loop() {
  ...
  //Clear the Bluetooth receiving char array every time
  for( int i = 0; i < sizeof(control_signal_bt);  i++ )
   control_signal_bt[i] = (char)0;
   
  //Control signal received over Bluetooth
